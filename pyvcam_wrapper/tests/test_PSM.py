...
if (cam.check_param(const.PARAM_SCAN_MODE)):
    ...
    print("Time to sequence = {0}".format(auto_seq_time_hdr_1))

    # Set camera to HDR Gain 2; Camera should still be in Auto mode
    print("Setting camera to HDR Gain 2")
    cam.speed_table_index = 1
    cam.gain = 2
    ...
    # Since numpy's mean can be inaccurate in single precision, computing in float64 instead, which is more accurate
    auto_mean_hdr_2 = np.mean(frames, dtype=np.float64)
    auto_seq_time_hdr_2 = end-start

    print("First five rows of frame: {}, {}, {}, {}, {}".format(*frames[0][:5]))
    print("Mean = {0} ADU".format(auto_mean_hdr_2))
    print("Time to sequence = {0}".format(auto_seq_time_hdr_2))

    # Get Line Delay Mode values
    ...
    if (cam.prog_scan_mode is not const.prog_scan_modes["Line Delay"]):
        print("Resetting psm to Line Delay")
    ...
    assert ld_seq_time > auto_seq_time_hdr_2, "Auto Mode sequence time was greater than Line Delay sequence time (HDRG2)"

    # Get Scan Width Mode values
    ...
    if (cam.prog_scan_mode is not const.prog_scan_modes["Scan Width"]):
        print("Resetting psm to Scan Width")
    ...
    assert sw_seq_time > auto_seq_time_hdr_2, "Auto Mode sequence time was greater than Scan Width sequence time (HDRG2)"
